...
background:#F5F5F5;"></label></div>
  ...
  <form style="text-align:center">
      ...
      <input type="button" style="width:200px" id="btnMovimientos" value="Movimientos" /></br></br>      
...
<div id="TransferenciaDialog" class="oculta" title="Transferencia">
  <form style="text-align:center">
  	  </br>
...
<script>
	$(function(){
		...
		dialog = $( "#MeterDineroDialog" ).dialog({
		  ...
		  buttons: {
			Aceptar: function() {
				if ($("#txtAumentarImporte").val()=="")
				{
					alert("Debes indicar un importe");
					return false;
				}	
		...
		$("#btnMovimientos" ).on("click",function(){
			location.href="movimientos.php?idcuenta="+idcuenta;
		});
	    ...
	    $("#MeterDineroDialog").keydown(function (event) {
	    ...
	    $("#SacarDineroDialog").keydown(function (event) {
	        if (event.keyCode == $.ui.keyCode.ENTER) {
	            //$(this).parent()	                   .find("button:eq(0)").trigger("click");
	            return false;
	        }
